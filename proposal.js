document.addEventListener('DOMContentLoaded', () => {
    ...
    // Показ формы при нажатии на кнопку "Вставьте текст"
    ...
    form.addEventListener('submit', (event) => {
        ...
        const textContent = inputArea.value.trim();
        if (textContent) {
            console.log('Submitting text:', textContent); // Debug message
            ...
            .then(blob => {
                console.log('Received blob:', blob); // Debug message
    ...
    // Обработчик для кнопки "Вставьте документ" (показать input file)
    ...
    document.getElementById('soundButton').addEventListener('click', function (event) {
        ...
        console.log('Collected text:', textContent); // Debug message
    ...
    // Остальной код для функционала страницы, если он необходим
    // Например, для управления доступностью, изменением размера шрифта и цветовых схем
